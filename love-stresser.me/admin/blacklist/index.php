...
body,div,span,h2,h3,ul,li,a,img,form,header,nav,section{margin:0;padding:0;border:0;outline:0;font-weight:inherit;font-style:inherit;font-size:100%;font-family:inherit;vertical-align:baseline;list-style:none;}
:focus{outline:0;}
...
.main-content{border-radius:24px;border:1px solid rgba(255, 255, 255, 0.08);backdrop-filter:blur(12px);padding:50px 0;width:1440px;height:1025px;margin-top:-1050px;justify-content:center;align-items:center;}
table{width:600px;border-collapse:separate;border-spacing:0;border:none;border-radius:15px;}
...
.action_button_a{border-radius:10px;padding:5px;font-style:normal;font-weight:600;font-size:18px;line-height:22px;color:#FFFFFF;border:none;height:30px;width:74px;background:linear-gradient(57.75deg, #F70FFF 14.44%, #2C63FF 85.65%);box-shadow:0 16px 24px rgba(247, 15, 255, 0.48);cursor:pointer;margin:0 auto;}
.adapt{width:400px;margin-bottom:15px;}
...
<header class="header">
    <div class="container">
        <div id="header__row" class="header__row">
            <a href="https://love-stresser.me/" class="header__logo">
                                  ...
                                  <br></br>
                                    <div class="sidebar-items">
                                          <ul>
                                            ...
                                            <li><a class="current_selected current-menu-item" href="">Blacklist</a></li>
...
if (isset($_POST['delete_host'])) {
    $hostToDelete = $_POST['delete_host'];
    $sql = "DELETE FROM blacklist WHERE host = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $hostToDelete);
    $stmt->execute();
    $stmt->close();
}

if (isset($_POST['new_host'])) {
    $newHost = normalize_host($_POST['new_host']);
    if (filter_var($newHost, FILTER_VALIDATE_DOMAIN, FILTER_FLAG_HOSTNAME) || filter_var($newHost, FILTER_VALIDATE_IP)) {
        $checkSql = "SELECT COUNT(*) AS count FROM blacklist WHERE host = ?";
        $checkStmt = $conn->prepare($checkSql);
        $checkStmt->bind_param("s", $newHost);
        $checkStmt->execute();
        $checkResult = $checkStmt->get_result();
        $checkRow = $checkResult->fetch_assoc();

        if ($checkRow['count'] == 0) {
            $sql = "INSERT INTO blacklist (host) VALUES (?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("s", $newHost);
            $stmt->execute();
            $stmt->close();
        } else {
            echo "<p>Host already exists in the blacklist.</p>";
        }

        $checkStmt->close();
    } else {
        echo "<p>Invalid host format.</p>";
    }
}
...
$offset = ($page - 1) * $hostsPerPage;

$totalHostsSql = "SELECT COUNT(*) AS total FROM blacklist";
$totalHostsResult = $conn->query($totalHostsSql);
$totalHostsRow = $totalHostsResult->fetch_assoc();
$totalHosts = $totalHostsRow['total'];

$totalPages = ceil($totalHosts / $hostsPerPage);

$sql = "SELECT * FROM blacklist LIMIT ?, ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $offset, $hostsPerPage);
$stmt->execute();
$result = $stmt->get_result();
?>
...
<section class="main-content">
    ...
    <br></br>
    <form method="post">
        <input class="adapt" type="text" name="new_host" placeholder="Enter domain or IP" required>
        <button style="border-radius: 10px;" type="submit">Add</button>
    </form>
    <br></br>
    <h2>Blacklisted Hosts</h2>
    <br></br>
    <table border="1">
        <tr>
            <th>Host</th>
            ...
            <tr>
                ...
                <td>
                    <form method="post" style="display:inline;">
                        <input type="hidden" name="delete_host" value="<?php echo htmlspecialchars($row['host']); ?>">
                        <button class="action_button_a" type="submit">Remove</button>
                    </form>
                </td>
            </tr>
        <?php } ?>
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $totalPages; $i++) { ?>
            <span class="pagination-link <?php if ($i == $page) echo 'current-page'; ?>">
