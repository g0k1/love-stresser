...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $stmt = $conn->prepare("INSERT INTO announcements (title, type, content, date) VALUES (:title, :type, :content, :date)");
        ...
        if ($stmt->execute()) {
            ...
            echo "<p>Error adding news. Please try again.</p>";
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_news'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<p>Error deleting news. Please try again.</p>";
    }
}

$stmt = $conn->prepare("SELECT id, title, type, content, date FROM announcements ORDER BY date DESC");
$stmt->execute();
...
<section class="main-content">
    ...
    <form method="POST" action="">
        ...
        <input type="submit" name="add_news" value="Add News" class="button-ga54">
    </form>
<br></br>
    <h2>Current News</h2>
    <?php if (count($news_entries) > 0): ?>
            ...
            <tbody>
                <?php foreach ($news_entries as $news): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($news['type']); ?></td>
